...
<html>
    ...
    $wynik = $conn-> query("SELECT * from odpowiedzi2_fifa");
    if($wynik->num_rows > 0){
    ...
    $db = "forum internetowe";
    $conn = new mysqli("localhost", "root", "", "forum internetowe") or die("Błąd");
    $zagadnienia=$_GET['id_zagadnienia2_fifa'];
    $nick=$_GET['nick'];
    $komentarze=$_GET['komentarze'];
    $dodaj_kom=mysqli_query($conn, "INSERT INTO odpowiedzi2_fifa (id_zagadnienia, nick, komentarze) value ('1','$nick', '$komentarze') ");
    $conn->close();
    ?>
            </div>
            <div id="footer">
