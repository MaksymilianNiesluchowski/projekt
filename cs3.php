...
<html>
    ...
    $wynik = $conn-> query("SELECT * from odpowiedzi3_cs");
    if($wynik->num_rows > 0){
    ...
    $db = "forum internetowe";
    $conn = new mysqli("localhost", "root", "", "forum internetowe") or die("Błąd");
    $zagadnienia=$_GET['id_zagadnienia3_cs'];
    $nick=$_GET['nick'];
    $komentarze=$_GET['komentarze'];
    $dodaj_kom=mysqli_query($conn, "INSERT INTO odpowiedzi3_cs (id_zagadnienia, nick, komentarze) value ('1','$nick', '$komentarze') ");
    $conn->close();
    ?>
            </div>
            <div id="footer">
